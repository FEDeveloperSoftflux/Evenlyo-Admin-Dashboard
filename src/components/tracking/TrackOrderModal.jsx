      ...
      >
        ...
        <div className="bg-gray-50 rounded-xl p-6 mb-6 flex flex-col md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-col items-end">
            ...
            <div className="text-md font-bold mt-6">Total Price: <span className="text-md">{order.totalPrice}</span></div>
          </div>
        </div>
              ...
              <div key={idx} className="flex items-start gap-4">
                <div className="flex flex-col items-center">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center mb-1
                    ${step.completed ? 'bg-green-300 text-white' : 'bg-gray-200 text-gray-400'}
                  `}>
                    <img src={step.icon} alt={step.title} className="w-4 h-4" />
                  </div>
                  {idx < order.timeline.length - 1 && (
                    <div className={`w-0.5 h-8 ${step.completed ? 'bg-green-300' : 'bg-gray-200'}`}></div>
                  )}
                </div>
                <div className="flex-1">
                  <div className={`font-medium text-sm ${step.completed ? 'text-green-600' : 'text-gray-700'}`}>{step.title}</div>
        ...
        <div className="bg-gray-50 rounded-xl p-4 mb-6">
          ...
          <div className="text-sm text-gray-600 flex items-center gap-2">
            <img src="/assets/Tracking.svg" alt="Progress" className="w-4 h-4" />
            {order.progressNote}
          </div>
        </div>
